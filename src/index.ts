...
const PrependClassifier = new Classifier([
...
const ControlClassifier = new Classifier([
...
const ExtendClassifier = new Classifier([
...
const SpacingMarkClassifier = new Classifier([
...
const LVClassifier = new Classifier([
...
const LVTClassifier = new Classifier([
...
const Extended_PictographicClassifier = new Classifier([
...
export class GraphemeSplitter {
	prevBreak(string: string, index: number) {
		...
		while(prevIndex >= 0) {
			const code = string.charCodeAt(prevIndex);
			// low
			if (0xDC00 <= code && code <= 0xDFFF) {
				prevIndex--;
				continue;
			...
			let nextBreak = this.nextBreak(string, prevIndex);

			if (nextBreak === index) {
				const prevCP = GSHelper.codePointAt(string, prevIndex);
				// [RI RI] *
				if (GraphemeSplitter.getGraphemeBreakProperty(prevCP) === 4 && prevIndex > 0) {
					let oneBackward = prevIndex - 1;
			...
			if (nextBreak < index) {
				...
				if (GraphemeSplitter.getGraphemeBreakProperty(prevCP) === 15 && prevIndex > 0) {
					...
					let newBreak = this.nextBreak(string, oneBackward);
