...
class GSHelper {
	 ...
	 * @returns {boolean}
	 */
	static isSurrogate(str: string, pos: number) {
	 ...
	 * @returns {number}
	 */
	static codePointAt(str: string, idx: number) {
		if (idx === undefined) {
			idx = 0;
		}
	 ...
	 * @returns {number}
	 */
	static shouldBreak(start: number, mid: number[], end: number, startEmoji: number, midEmoji: number[], endEmoji: number) {
...
export class GraphemeSplitter {
	 ...
	 * @returns {number}
	 */
	nextBreak(string: string, index: number) {
		if (index === undefined) {
			index = 0;
		}
		if (index < 0) {
	 ...
	 * @returns {number}
	 */
	static getGraphemeBreakProperty(code: number) {
		...
		if (
			SpacingMarkClassifier.match(code)
		) {
			return SpacingMark;
		}
		if (
	...
	static getEmojiProperty(code: number) {
		...
		// node ./scripts/generate.js
		if (
			Extended_PictographicClassifier.match(code)
		) {
